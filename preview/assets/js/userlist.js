...
(function($){
    ...
    userF.prototype.init = function(){
      ...
      userF.prototype.appentList = function(data){
            ...
            var uid =data.uid;
            var html = '\
                         <div style="padding:10px;border-bottom:1px solid gray;" data-uid="'+ uid +'" data-ext="'+ data.ext +'" data-client="' + data.clientId + '" time="'+row+'" id="'+uid+'" class="'+this.opts.itemClass+'" >\
                             <div class="user-item-info"><img style="width:35px;height:35px;border-radius:50%;margin-right:10px;" src="'+data.ava+'" />\
                             '+this.opts.config.htmlspecialchars(nick)+ (data.ext=='teacher' ? "（老师）":'') + '\
                             </div>\
                         </div>\
            ';
